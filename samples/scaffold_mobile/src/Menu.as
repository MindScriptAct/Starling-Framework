package
{
    ...
    public class Menu extends SpriteStarling
    {
        ...
        {
            var textField:TextFieldStarling = new TextFieldStarling(250, 50, "Game Scaffold",
                "Desyrel", BitmapFontStarling.NATIVE_SIZE, 0xffffff);
            textField.x = (Constants.STAGE_WIDTH - textField.width) / 2;
            textField.y = 50;
            addChild(textField);
            
            var button:ButtonStarling = new ButtonStarling(Root.assets.getTexture("button_normal"), "Start");
            button.fontName = "Ubuntu";
            button.fontSize = 16;
            button.x = int((Constants.STAGE_WIDTH - button.width) / 2);
            button.y = Constants.STAGE_HEIGHT * 0.75;
            button.addEventListener(EventStarling.TRIGGERED, onButtonTriggered);
            addChild(button);
        }
