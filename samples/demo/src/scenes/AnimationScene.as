package scenes
{
    ...
    public class AnimationScene extends Scene
    {
        ...
        private var mDelayButton:ButtonStarling;
        private var mEgg:ImageStarling;
        private var mTransitionLabel:TextFieldStarling;
        private var mTransitions:Array;
        
        public function AnimationScene()
        {
            mTransitions = [TransitionsStarling.LINEAR, TransitionsStarling.EASE_IN_OUT,
                            TransitionsStarling.EASE_OUT_BACK, TransitionsStarling.EASE_OUT_BOUNCE,
                            TransitionsStarling.EASE_OUT_ELASTIC];
            
            var buttonTexture:TextureStarling = Game.assets.getTexture("button_normal");
            
            // create a button that starts the tween
            mStartButton = new ButtonStarling(buttonTexture, "Start animation");
            ...
            mDelayButton = new ButtonStarling(buttonTexture, "Delayed call");
            mDelayButton.addEventListener(EventStarling.TRIGGERED, onDelayButtonTriggered);
            mDelayButton.x = mStartButton.x;
            mDelayButton.y = mStartButton.y + 40;
            addChild(mDelayButton);
            
            // the Starling will be tweened
            mEgg = new ImageStarling(Game.assets.getTexture("starling_front"));
            addChild(mEgg);
            resetEgg();
            
            mTransitionLabel = new TextFieldStarling(320, 30, "", "Verdana", 20, 0, true);
        ...
        private function onStartButtonTriggered():void
        {
            ...
            var tween:TweenStarling = new TweenStarling(mEgg, 2.0, transition);
            ...
            tween.animate("rotation", deg2radStarling(90)); // conventional 'animate' call
            ...
            StarlingStarling.juggler.add(tween);
            
            // show which tweening function is used
            mTransitionLabel.text = transition;
            mTransitionLabel.alpha = 1.0;
            
            var hideTween:TweenStarling = new TweenStarling(mTransitionLabel, 2.0, TransitionsStarling.EASE_IN);
            hideTween.animate("alpha", 0.0);
            StarlingStarling.juggler.add(hideTween);
        }
        ...
        private function onDelayButtonTriggered():void
        {
            ...
            StarlingStarling.juggler.delayCall(colorizeEgg, 1.0, true);
            StarlingStarling.juggler.delayCall(colorizeEgg, 2.0, false);
            StarlingStarling.juggler.delayCall(function():void { mDelayButton.enabled = true; }, 2.0);
        }
        
        private function colorizeEgg(colorize:Boolean):void
        {
            mEgg.color = colorize ? ColorStarling.RED : ColorStarling.WHITE;
        }
        
        public override function dispose():void
        {
            mStartButton.removeEventListener(EventStarling.TRIGGERED, onStartButtonTriggered);
            mDelayButton.removeEventListener(EventStarling.TRIGGERED, onDelayButtonTriggered);
            super.dispose();
        }
    }
}